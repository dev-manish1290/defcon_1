        ...
        "columns": [
            ...
            if(row.uc_master_corporate_action_id != null){
                if(row.qc_status=="True"){
                    AddPreApproved_CA(row.uc_master_corporate_action_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.gross_amount_uc,row.currency_uc,row.record_date_uc,row.payment_date_uc,row.user_comment);
            }
                    return '<h8>Approved</h8>';
                }
                else{
             if(row.qc_status=="True"){
                        AddPreApproved_CA(row.tr_master_corporate_action_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.gross_amount_tr,row.currency_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
                    }
            return '<h8>Approved</h8>';
            }},
...
function AddPreApproved_CA(master_corporate_action_id,global_id,ric,mic,isin,master_ca_type,exdate,grossamount,currency,record_date,payment_date,comment_qc){
    var someData = [{
...
function newv(check,d,ca,mca){
    ...
    var someData = [
        ...
        {"master_corporate_action_id":dd,"global_event_id": d, "ca_id": ca,"analyst_status": "unknown","source": source_value, "ric": ric_value, "mic": mic_value, "isin":isin_value, "master_ca_type":catype_value, "exdate":ex_value,"grossamt":gross_value,"currency":curr_value,"record_date":record_value,"payment_date":payment_value}
     ];
...
function confirm_analyst_ca(check,data){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data, "ca_id": null, "qc_status": "True","source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"grossamt":null,"currency":null,"record_date":null,"payment_date":null}
         ];
         var final_data = saved_data.concat(someData);
        localStorage.setItem("updated_data",JSON.stringify(final_data));
        console.log(JSON.parse(localStorage.getItem("updated_data")));
...
function invalidate_analyst_ca(check,data,comment_qc){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data,"comment": comment_qc, "ca_id": null, "qc_status": "False","source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"grossamt":null,"currency":null,"record_date":null,"payment_date":null}
         ];
         var final_data = saved_data.concat(someData);
        localStorage.setItem("updated_data",JSON.stringify(final_data));
    }
...
document.getElementById("modal-content-msg").style.display='block';
var text = document.getElementById("commentdata");
text.innerHTML = "<label>Add Comment</label><br><br><input type='hidden' id='comment_gc' value='"+check+"'><input type ='text' name= 'comm_ca' id='comm_ca' style='width:90%; height:40px;'>";
var divdata = document.getElementById("modalfooter");
...
function Addcomment_ca(data){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data, "ca_id": null, "qc_status": "True","comment":comm,"source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"grossamt":null,"currency":null,"record_date":null,"payment_date":null}
            ]
            var final_data = saved_data.concat(someData);
            localStorage.setItem("updated_data",JSON.stringify(final_data));
    }
