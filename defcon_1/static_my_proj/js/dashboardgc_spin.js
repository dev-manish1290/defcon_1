function format ( d ) {
    ...
    if(d.tr_master_corporate_action_id !=null){
        ...
        '<td id="'+d.tr_ca_id+'f10" style="white-space: normal;">' +  d.spunoff_isin_tr + '</td>'+ 
    ...
    if(d.edi_master_corporate_action_id !=null){
        ...
        '<td id="'+d.edi_ca_id+'f10" style="white-space: normal;">' +  d.spunoff_isin_edi + '</td>'+ 
    ...
    if(d.fs_master_corporate_action_id !=null){
        ...
        '<td id="'+d.fs_ca_id+'f10" style="white-space: normal;">' +  d.spunoff_isin_fs + '</td>'+ 
...
var table = $('#example').DataTable( {
    ...
    "ajax": {
        ...
        "columns": [
            ...
            { "data": "spunoff_name_gc" },
            { "data": "spunoff_isin_gc"},
            {
                render: function ( data, type, row ) {
            ...
            render: function ( data, type, row ) {
                if(row.uc_master_corporate_action_id != null){
                    if(row.qc_status=="True"){
                        AddPreApproved_CA(row.uc_master_corporate_action_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.ratio_uc,row.spunoff_name_uc,row.spunoff_isin_uc,row.record_date_uc,row.payment_date_uc,row.user_comment);
                    }
            ...
            // }
             if(row.qc_status=="True"){
                AddPreApproved_CA(row.tr_master_corporate_action_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.ratio_tr,row.spunoff_name_tr,row.spunoff_isin_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
                    }
            return '<h8>Approved</h8>';}},
       ...
       {  className: "state",
            ...
            render: function ( data, type, row ) {
                ...
                AddPreApproved_CA(master_ca_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.ratio_tr,row.spunoff_name_tr,row.spunoff_isin_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
            }
            return row.state
        }
    }
    ...
    if ((row.data().spunoff_name_tr == row.data().spunoff_name_edi) && (row.data().spunoff_name_tr == row.data().spunoff_name_fs)){
        row.data().spunoff_name_gc=row.data().spunoff_name_tr;
    }
    else{
        row.data().spunoff_name_gc='';
    }
    document.getElementById("SpunOFF_Name").value = row.data().spunoff_name_gc;
    if ((row.data().spunoff_isin_tr == row.data().spunoff_isin_edi) && (row.data().spunoff_isin_tr == row.data().spunoff_isin_fs)){
        row.data().spunoff_isin_gc=row.data().spunoff_isin_tr;
    }
    else{
        row.data().spunoff_isin_gc='';
    }
    document.getElementById("SpunOFF_Ticker").value = row.data().spunoff_isin_gc;
    ...
    $("#editform").submit(function(e) {
        ...
        var formData = {
            ...
            'spunoff_name_gc'             : $('input[name=SpunOFF_Name]').val(),
            'spunoff_isin_gc'             : $('input[name=SpunOFF_Ticker]').val(),
...
function AddPreApproved_CA(master_corporate_action_id,global_id,ric,mic,isin,master_ca_type,exdate,ratio_gc,spunoff_name_gc,spunoff_isin_gc,record_date,payment_date,comment_qc){
    var someData = [
        {"master_corporate_action_id":master_corporate_action_id,"global_event_id": global_id, "ca_id": null, "comment":comment_qc,"qc_status": "True","source": null, "ric": ric, "mic": mic, "isin":isin, "master_ca_type":master_ca_type, "exdate":exdate,"ratio_gc":ratio_gc,"spunoff_name_gc":spunoff_name_gc,"spunoff_isin_gc":spunoff_isin_gc,"record_date":record_date,"payment_date":payment_date,"state":"Pre Approved"}
     ];
    var saved_data = JSON.parse(localStorage.getItem("updated_data"));
...
function newv(check,d,ca,mca){
    ...
    var someData = [
        ...
        {"master_corporate_action_id":dd,"global_event_id": d, "ca_id": ca,"analyst_status": "unknown","source": source_value, "ric": ric_value, "mic": mic_value, "isin":isin_value, "master_ca_type":catype_value, "exdate":ex_value,"ratio_gc":ratio_value,"spunoff_name_gc":spunoff_name_value,"spunoff_isin_gc":spunoff_isin_value,"currency":curr_value,"record_date":record_value,"payment_date":payment_value}
     ];
...
function confirm_analyst_ca(check,data){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data, "ca_id": null, "qc_status": "True","source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"ratio_gc":null,"spunoff_name_gc":null,"spunoff_isin_gc":null,"currency":null,"record_date":null,"payment_date":null}
...
function invalidate_analyst_ca(check,data,comment_qc){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data,"comment": comment_qc, "ca_id": null, "qc_status": "False","source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"ratio_gc":null,"spunoff_name_gc":null,"spunoff_isin_gc":null,"currency":null,"record_date":null,"payment_date":null}
...
function Addcomment_ca(data){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data, "ca_id": null,"qc_status": "True","comment":comm,"source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"ratio_gc":null,"spunoff_name_gc":null,"spunoff_isin_gc":null,"currency":null,"record_date":null,"payment_date":null}
