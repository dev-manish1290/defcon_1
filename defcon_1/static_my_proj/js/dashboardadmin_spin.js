         ...
         {  className: "state",
            ...
            render: function ( data, type, row ) {
                ...
                if(row.uc_master_corporate_action_id != null){
                    ...
                    AddPreApproved_CA(user_ca_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.ratio_uc,row.spunoff_name_uc,row.spunoff_isin_uc,row.currency_uc,row.record_date_uc,row.payment_date_uc,row.user_comment);
                    return "Pre Approved"
                    }
                }
                else{
                ...
                if (row.state == null){
                    ...
                    if(row.state=="Pre Approved"){
                        AddPreApproved_CA(master_ca_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.ratio_tr,row.spunoff_name_tr,row.spunoff_isin_tr,row.currency_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
                    }
                    return row.state
                }
            }
...
function AddPreApproved_CA(master_corporate_action_id,global_id,ric,mic,isin,master_ca_type,exdate,ratio_gc,spunoff_name_gc,spunoff_isin_gc,currency,record_date,payment_date,comment_a){
    var someData = [
        {"master_corporate_action_id":master_corporate_action_id,"global_event_id": global_id, "ca_id": null, "comment":comment_a,"admin_status": "True","source": null, "ric": ric, "mic": mic, "isin":isin, "master_ca_type":master_ca_type, "exdate":exdate,"ratio_gc":ratio_gc,"spunoff_name_gc":spunoff_name_gc,"spunoff_isin_gc":spunoff_isin_gc,"currency":currency,"record_date":record_date,"payment_date":payment_date,"state":"Pre Approved"}
     ];
    var saved_data = JSON.parse(localStorage.getItem("updated_data"));
...
function newv(check,d,ca,mca){
    ...
    var someData = [
        ...
        {"master_corporate_action_id":dd,"global_event_id": d, "ca_id": ca, "comment":null,"analyst_status": "unknown","source": source_value, "ric": ric_value, "mic": mic_value, "isin":isin_value, "master_ca_type":catype_value, "exdate":ex_value,"ratio_gc":ratio_value,"spunoff_name_gc":spunoff_name_value,"spunoff_isin_gc":spunoff_isin_value,"record_date":record_value,"payment_date":payment_value}
     ];
...
function confirm_analyst_ca(check,data,comment_analyst){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data,"comment":comment_analyst, "ca_id": null, "admin_status": "True","source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"ratio_gc":null,"spunoff_name_gc":null,"spunoff_isin_gc":null,"currency":null,"record_date":null,"payment_date":null}
         ];
         var final_data = saved_data.concat(someData);
        localStorage.setItem("updated_data",JSON.stringify(final_data));
        console.log(JSON.parse(localStorage.getItem("updated_data")));
...
function invalidate_analyst_ca(check,data,comment_analyst){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data,"comment":comment_analyst, "ca_id": null,"admin_status": "False","source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"spunoff_name_gc":null,"spunoff_isin_gc":null,"ratio_gc":null,"currency":null,"record_date":null,"payment_date":null}
         ];
         var final_data = saved_data.concat(someData);
        localStorage.setItem("updated_data",JSON.stringify(final_data));
    }
...
function Addcomment_ca(data){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"global_event_id": data, "ca_id": null,"admin_status":"unknown","comment":comm,"source": null, "ric": null, "mic": null, "isin":null, "master_ca_type":null, "exdate":null,"spunoff_name_gc":null,"spunoff_isin_gc":null,"ratio_gc":null,"payment_date":null}
            ]
            var final_data = saved_data.concat(someData);
            localStorage.setItem("updated_data",JSON.stringify(final_data));
    }
