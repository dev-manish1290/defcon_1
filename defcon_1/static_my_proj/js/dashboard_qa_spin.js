    ...
    "ajax": {
        ...
        "columns": [
            ...
            // }
                ...
                AddPreApproved_CA(row.tr_master_corporate_action_id,row.global_event_id,
                    row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.ratio_tr,
                    row.spunoff_name_tr,row.spunoff_isin_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
       ...
       {  className: "state",
            ...
            render: function ( data, type, row ) {
                ...
                if(row.uc_master_corporate_action_id != null){
                    ...
                    AddPreApproved_CA(user_ca_id,row.global_event_id,row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.ratio_uc,row.spunoff_name_uc,row.spunoff_isin_uc,row.record_date_uc,row.payment_date_uc,row.user_comment);
                    return "Pre Approved"
                    }
                }
                else{
...
function AddPreApproved_CA(master_corporate_action_id,global_id,ric,mic,isin,master_ca_type,exdate,ratio_gc,spunoff_name_gc,spunoff_isin_gc,record_date,payment_date,comment_admin){
    var someData = [{
        ...
        "exdate":exdate,
        "ratio_gc":ratio_gc,
        "spunoff_name_gc":spunoff_name_gc,
        "spunoff_isin_gc":spunoff_isin_gc,
...
function newv(check,d,ca,mca){
    ...
    var someData = [{
        ...
        "mic": mic_value, 
        "isin":isin_value, 
        "master_ca_type":catype_value, 
        "exdate":ex_value,
        "ratio_gc":ratio_value,
        "spunoff_name_gc":spunoff_name_value,
        "spunoff_isin_gc":spunoff_isin_value,
        "currency":curr_value,
...
function confirm_analyst_ca(check,data){
    ...
    if(i!==-1){
        ...
        var someData = [{
            ...
            "exdate":null,
            "ratio_gc":null,
            "spunoff_name_gc":null,
            "spunoff_isin_gc":null,
            "currency":null,
            "record_date":null,
            "payment_date":null
        }];
