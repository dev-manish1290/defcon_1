...
$('#submitCA').click(function(){
    ...
    if(i!==-1){
        ...
        // var r =  confirm('Do you want to submit '+data_length+' pre approved CA and '+data_length+' manual approved CA ?');
        if(r==true){
         var formData={
...
var table = $('#example').DataTable( {
        ...
        "columns": [
            ...
            // }
                        ...
                        AddPreApproved_CA(row.tr_master_corporate_action_id,row.global_event_id,
                            row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,
                            row.gross_amount_tr,row.currency_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
	...
	$('#example tbody').on('click', 'td.details-control', function () {
        ...
        if ( row.child.isShown() ) {
            ...
            cancel = "../static/images/cancel.png";
            value= '<a onclick="invalidate_qc_ca(this,'+row.data().global_event_id+',\'' + row.data().user_comment + '\')"><img src="' +cancel+'"></a>';
...
function AddPreApproved_CA(master_corporate_action_id,global_id,ric,mic,isin,master_ca_type,exdate,grossamount,currency,record_date,payment_date,comment_admin){
    var someData = [{
...
function invalidate_qc_ca(check,data,comment_admin){
    $('input[name="predata"]').val("true");
