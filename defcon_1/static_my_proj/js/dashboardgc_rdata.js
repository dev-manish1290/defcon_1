                    ...
                    if(row.uc_master_corporate_action_id != null){
                        if(row.qc_status=="True"){
                            AddPreApproved_CA(row.uc_master_corporate_action_id,row.global_event_id,
                                row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.old_value_uc,row.new_value_uc,row.record_date_uc,row.payment_date_uc,row.user_comment);
                        }
                ...
                // }
                 if(row.qc_status=="True"){
                            AddPreApproved_CA(row.tr_master_corporate_action_id,row.global_event_id,
                                row.ric,row.mic,row.isin,row.master_ca_type,row.master_effective_date,row.old_value_tr,row.new_value_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
                        }
                return '<h8>Approved</h8>';}},
            ...
            render: function ( data, type, row ) {
                ...
                && ((row.tr_effective_date == row.fs_effective_date) && (row.fs_effective_date == row.edi_effective_date) && (row.tr_effective_date == row.edi_effective_date))){
                    ...
                    if(row.state=="Pre Approved"){
                        AddPreApproved_CA(master_ca_id,row.global_event_id,row.ric,row.mic,row.isin,
                            row.master_ca_type,row.master_effective_date,row.old_value_tr,row.new_value_tr,row.record_date_tr,row.payment_date_tr,row.user_comment);
                    }
                    return row.state
                }
            }
...
function AddPreApproved_CA(master_corporate_action_id,global_id,ric,mic,isin,master_ca_type,exdate,old_value_gc,new_value_gc,record_date,payment_date,comment_qc){
    var someData = [
        {"master_corporate_action_id":master_corporate_action_id,"global_event_id": global_id, "qc_status": "True","ca_id": null, "comment":comment_qc, "source": null, "ric": ric, "mic": mic, "isin":isin, "master_ca_type":master_ca_type, "exdate":exdate,"old_value_gc":old_value_gc,"new_value_gc":new_value_gc,"record_date":record_date,"payment_date":payment_date,"state":"Pre Approved"}
     ];
    var saved_data = JSON.parse(localStorage.getItem("updated_data"));
