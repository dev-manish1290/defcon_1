function format ( d ) {
    ...
    if(d.currency_tr !==null || d.record_date_tr !==null || d.payment_date_tr!==null){
        table +=  '<input type="hidden" id="'+d.tr_master_corporate_action_id+'m0" value=' + d.tr_master_corporate_action_id + '><tr id="'+d.global_event_id+'" ><td><input type="checkbox" id="1" name="'+d.global_event_id+'" onclick="newv(this,'+d.global_event_id+','+d.tr_ca_id+','+d.tr_master_corporate_action_id+')"></td>' +
    ...
    {
        table += '<input type="hidden" id="'+d.edi_master_corporate_action_id+'m0" value=' + d.edi_master_corporate_action_id + '><tr id="'+d.global_event_id+'"><td><input type="checkbox" id="2" name="'+d.global_event_id+'" onclick="newv(this,'+d.global_event_id+','+d.edi_ca_id+','+d.edi_master_corporate_action_id+')"></td>' +
        ...
        '<td id="'+d.edi_ca_id+'f4" style="white-space: normal;">' +  d.ca_type + '</td>' + 
        '<td id="'+d.edi_ca_id+'f5" style="white-space: normal;">' +  d.effective_date +'</td>' + 
    ...
    if(d.currency_fs !==null || d.record_date_fs !==null || d.payment_date_fs!==null){
        table += '<input type="hidden" id="'+d.fs_master_corporate_action_id+'m0" value=' + d.fs_master_corporate_action_id + '><tr id="'+d.global_event_id+'"><td><input type="checkbox" id="3" name="'+d.global_event_id+'" onclick="newv(this,'+d.global_event_id+','+d.fs_ca_id+','+d.fs_master_corporate_action_id+')"></td>' +
            ...
            '<td id="'+d.fs_ca_id+'f4" style="white-space: normal;">' +  d.ca_type + '</td>' + 
            '<td id="'+d.fs_ca_id+'f5" style="white-space: normal;">' +  d.effective_date +'</td>' + 
    ...
    "ajax": {
        ...
        "columns": [
            ...
            { className: "gross_amount",
                data: null,
            render: function ( data, type, row ) {
                if((row.gross_amount_edi == row.gross_amount_fs) && (row.gross_amount_fs == row.gross_amount_tr))
                 {
                     row.GrossAmt=row.gross_amount_edi;
                    return row.gross_amount_edi;
                 }
                 else{
                    row.GrossAmt="";
                     return "";
...
function AddPreApproved_CA(master_corporate_action_id,global_id,ric,mic,isin,ca_type,exdate,grossamount,currency,record_date,payment_date){
    ...
    var someData = [
        {"master_corporate_action_id":master_corporate_action_id,"global_event_id": global_id, "ca_id": null, "comment":null,"analyst_status": "True","source": null, "ric": ric, "mic": mic, "isin":isin, "ca_type":ca_type, "exdate":exdate,"grossamt":grossamount,"currency":currency,"record_date":record_date,"payment_date":payment_date,"state":"Pre Approved"}
     ];
    var saved_data = JSON.parse(localStorage.getItem("updated_data"));
...
function invalidate_analyst_ca(check,data,master_id){
    ...
    if(i!==-1){
        ...
        var someData = [
            {"master_corporate_action_id":master_id,"global_event_id": data, "ca_id": null, "comment":null,"analyst_status": "False","source": null, "ric": $('input[name=ric]').val(), "mic": $('input[name=mic]').val(), "isin":$('input[name=isin]').val(),"ca_type":null, "exdate":null,"grossamt":null,"currency":null,"record_date":null,"payment_date":null}
         ];
         var final_data = saved_data.concat(someData);
        localStorage.setItem("updated_data",JSON.stringify(final_data));
    }
